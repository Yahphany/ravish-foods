...
export default function VendorDashboard() {
  ...
  const { dbMenuItems, addMenuItem, toggleMenuItemAvailability } = useContext(MenuContext);
  ...
  const vendorItems = dbMenuItems.filter(item => item.vendor_id === session?.user?.id);

  // Vendor orders (since checkout groups orders by vendor, we can filter orders by vendorId)
  const vendorOrders = orders.filter(order => order.vendorId === session?.user?.id);

  // Metrics computation
  const activeOrders = vendorOrders.filter(o => o.status === "Pending" || o.status === "Preparing");
  const readyOrders = vendorOrders.filter(o => o.status === "Ready for Delivery" || o.status === "Out for Delivery");
  const completedOrdersCount = vendorOrders.filter(o => o.status === "Completed").length;
  ...
  const handleAddItem = async (e) => {
    ...
    try {
      const result = await addMenuItem({
        ...
        category,
        description,
        image: image || "https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=600&auto=format&fit=crop", // high-res default placeholder
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-5 grid-cols-2 lg:grid-cols-4">
        <div className="rounded-3xl bg-white p-6 shadow-sm border border-black/[0.02] flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Kitchen Queue</p>
        ...
        <div className="rounded-3xl bg-white p-6 shadow-sm border border-black/[0.02] flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Ready / Shipping</p>
            <p className="mt-1 text-2xl font-black text-gray-900">{readyOrders.length}</p>
          </div>
        </div>
        ...
        <div className="rounded-3xl bg-white p-6 shadow-sm border border-black/[0.02] flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-2xl font-black text-gray-900">{completedOrdersCount}</p>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="rounded-3xl bg-white p-8 shadow-sm border border-black/[0.03]">
            <div className="flex items-center gap-2.5 mb-6 border-b border-gray-100 pb-4">
              ...
              <h2 className="text-2xl font-bold text-gray-900">Active Incoming Orders</h2>
            </div>
            ...
            {ordersLoading && vendorOrders.length === 0 ? (
              ...
              <div className="text-center py-12 flex flex-col items-center">
                ...
                <p className="text-sm text-gray-500 mt-1">When buyers order your dishes, they will appear here in real-time.</p>
              </div>
            ) : (
                  ...
                  <div key={order.id} className="rounded-2xl border border-gray-100 p-6 bg-gray-50/50 hover:bg-gray-50 transition-colors duration-200">
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-gray-100 pb-4 mb-4">
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span className={`rounded-full px-3 py-1 text-xs font-bold ${
                            ...
                            {order.status === "Preparing" ? "Preparing (Packaging)" : order.status}
                          </span>
                        </div>
                      ...
                      <div className="text-left sm:text-right">
                        ...
                        <p className="text-lg font-black text-[#F24E05]">₦{order.total.toFixed(2)}</p>
                      </div>
                    </div>

                    {/* Order items */}
                    <div className="mb-4">
                      <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Requested Dish</p>
                      <div className="flex flex-col gap-2">
                        {order.items.map((item, idx) => (
                          <div key={idx} className="flex items-center justify-between text-sm bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
                            ...
                            <span className="font-extrabold text-gray-900 text-base">x{item.quantity}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="grid gap-3 sm:grid-cols-2 text-xs text-gray-600 bg-white p-4 rounded-xl border border-gray-100 mb-5">
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          ...
                          <p className="mt-0.5">{order.customer?.displayName || "Anonymous"}</p>
                    ...
                    {order.status === "Pending" && (
                      <div className="flex flex-col gap-3">
                        <p className="text-xs text-yellow-700 bg-yellow-50 border border-yellow-100 p-3.5 rounded-xl font-medium">
                          ⚠️ <strong>Action Required:</strong> A buyer has requested this food. Accept this order to let them know you are packaging the food.
                        </p>
                        <button
                          type="button"
                    ...
                    {order.status === "Preparing" && (
                      <div className="flex flex-col gap-3">
                        <p className="text-xs text-orange-700 bg-orange-50 border border-orange-100 p-3.5 rounded-xl font-medium">
                          🍳 <strong>Kitchen Status:</strong> Please package the requested food item. Once fully packed and ready for delivery, click the button below to notify Logistics.
                        </p>
                        <button
                          type="button"
                    ...
                    {order.status === "Ready for Delivery" && (
                      <div className="text-xs text-blue-700 bg-blue-50 border border-blue-100 p-3.5 rounded-xl font-medium">
                        ⏳ <strong>Awaiting Logistics:</strong> Order marked as packaged. Waiting for a dispatch rider to accept the delivery route.
                      </div>
                    )}

                    {order.status === "Out for Delivery" && (
                      <div className="text-xs text-indigo-700 bg-indigo-50 border border-indigo-100 p-3.5 rounded-xl font-medium">
                        🛵 <strong>On the Way:</strong> A dispatch rider has picked up the food and is delivering it to the customer.
                      </div>
                    )}
                    ...
                    {order.status === "Completed" && (
                      <div className="text-xs text-green-700 bg-green-50 border border-green-100 p-3.5 rounded-xl font-medium flex items-center gap-2">
                        <CheckCircle2 size={14} /> <strong>Order Completed:</strong> Food delivered successfully! Earnings added.
                      </div>
                    )}
                  </div>
                ))}
          ...
          <div className="rounded-3xl bg-white p-8 shadow-sm border border-black/[0.03]">
            ...
            <form onSubmit={handleAddItem} className="space-y-4 text-left">
              <div className="flex flex-col gap-1.5">
                <label htmlFor="foodName" className="text-xs font-bold text-gray-500 uppercase tracking-wider">Food Name</label>
                <input
                  type="text"
                  id="foodName"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  <label htmlFor="foodPrice" className="text-xs font-bold text-gray-500 uppercase tracking-wider">Price (₦)</label>
                  <input
                    type="number"
                    id="foodPrice"
                ...
                <div className="flex flex-col gap-1.5">
                  <label htmlFor="foodCategory" className="text-xs font-bold text-gray-500 uppercase tracking-wider">Category</label>
                  <select
                    id="foodCategory"
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor="foodDesc" className="text-xs font-bold text-gray-500 uppercase tracking-wider">Description</label>
                <textarea
                  id="foodDesc"
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor="foodImg" className="text-xs font-bold text-gray-500 uppercase tracking-wider">Image URL (Optional)</label>
                <input
                  type="url"
                  id="foodImg"
              ...
              >
                {adding ? "Listing Food..." : <><Plus size={16} /> Publish Food Listing</>}
              </button>
            </form>
          </div>

          {/* Vendor's active listings */}
          <div id="inventory" className="rounded-3xl bg-white p-8 shadow-sm border border-black/[0.03]">
            <div className="flex items-center gap-2 mb-6 border-b border-gray-100 pb-4">
              <Layers className="text-[#F24E05]" size={22} />
              <h2 className="text-2xl font-bold text-gray-900">Your Menu Items ({vendorItems.length})</h2>
            </div>
            ...
            {vendorItems.length === 0 ? (
              ...
              <div className="space-y-4">
                {vendorItems.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-3.5 p-3 rounded-2xl border border-gray-50 bg-gray-50/30 hover:bg-gray-50 transition">
                    <img src={item.image} alt={item.name} className="w-12 h-12 rounded-xl object-cover" />
                    <div className="flex-1 text-left min-w-0">
                      <h4 className="font-bold text-sm text-gray-900 truncate">{item.name}</h4>
                      <p className="text-xs text-gray-400 truncate capitalize">{item.category}</p>
                      <p className="font-extrabold text-sm text-[#F24E05] mt-0.5">₦{item.price.toFixed(2)}</p>
                    </div>
