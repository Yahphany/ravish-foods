  ...
  if (session && userRole === "vendor") {
    return (
      <div className="flex justify-between items-center py-4 px-6 md:px-20 bg-white fixed top-0 left-0 w-full text-black text-sm shadow-sm z-50 border-b border-gray-100">
        <Link
          to="/dashboard"
          className="font-extrabold text-xl tracking-wider text-[#F24E05]"
        >
          RAVISH{" "}
          <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider ml-1.5 px-2.5 py-0.5 bg-gray-100 rounded-full align-middle">
            Vendor
          </span>
        </Link>
        <div className="flex items-center space-x-6">
          <Link
            to="/dashboard"
            className="font-bold text-gray-600 hover:text-[#F24E05] transition-colors"
          >
            Dashboard
          </Link>
          <a
          ...
          <div className="p-1 text-gray-600 flex items-center gap-1 font-semibold border-l border-gray-200 pl-4">
            <User size={20} />
            <span className="hidden md:inline text-xs text-gray-500 max-w-[100px] truncate">
              {session.user.email}
            </span>
          </div>
    ...
    return (
      <div className="flex justify-between items-center py-4 px-6 md:px-20 bg-white fixed top-0 left-0 w-full text-black text-sm shadow-sm z-50 border-b border-gray-100">
        ...
        <div className="flex items-center space-x-6">
          ...
          <div
            className="relative p-1 text-gray-600 hover:text-[#F24E05] transition-colors cursor-pointer"
            title="Prepared Deliveries Feed"
          >
            <Bell size={20} />
  ...
  return (
    <div className="flex justify-between items-center py-4 px-6 md:px-20 bg-white fixed top-0 left-0 w-full text-black text-sm shadow-sm z-50 border-b border-gray-100">
      ...
      <div className="flex items-center space-x-6">
        <Link
          to="/"
          className="font-semibold text-gray-600 hover:text-[#F24E05] transition-colors"
        >
          Home
        </Link>
        <Link
        ...
        <Link
          to="/cart"
          className="relative p-1 text-gray-600 hover:text-[#F24E05] transition-colors"
        >
        ...
        >
          <User size={20} />
          {session && (
            <span className="hidden sm:inline text-xs text-gray-500 max-w-[80px] truncate">
              {session.user.email}
            </span>
          )}
        </Link>
